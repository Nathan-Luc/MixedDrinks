...
antlrcpp::Any Pass2Visitor::visitProgram(munchiesParser::ProgramContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitBlock(munchiesParser::BlockContext *ctx)
{
   auto value = visitChildren(ctx);
...
antlrcpp::Any Pass2Visitor::visitStmt(munchiesParser::StmtContext *ctx)
{
   j_file << endl << "; " + ctx->getText() << endl << endl;
   return visitChildren(ctx);
}
antlrcpp::Any Pass2Visitor::visitDeclare_stmt(munchiesParser::Declare_stmtContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitAssign_stmt(munchiesParser::Assign_stmtContext *ctx)
{
	cout << "--> in Assign()" << endl;
	auto value = visit(ctx->expr());
...
antlrcpp::Any Pass2Visitor::visitStr(munchiesParser::StrContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitPrint_stmt(munchiesParser::Print_stmtContext *ctx)
{
	cout << "--> in Print()" << endl;
	...
	else
	{
		   ...
		   j_file << "\t\tgetstatic\tjava/lang/System/out Ljava/io/PrintStream;" << endl;
		   j_file << "\t\tldc " << ctx->output()->str()->getText() << "\n" << endl;
		   j_file << "\t\tinvokevirtual java/io/PrintStream.print(Ljava/lang/String;)V" << endl;
	}
	   return NULL;
}
antlrcpp::Any Pass2Visitor::visitAddSubExpr(munchiesParser::AddSubExprContext *ctx)
{
	...
	string op = ctx->add_and_sub_op()->getText();
	string j_code;
...
antlrcpp::Any Pass2Visitor::visitMulDivExpr(munchiesParser::MulDivExprContext *ctx)
{
   auto value = visitChildren(ctx);
   ...
   string op = ctx->mul_and_div_op()->getText();
   string opcode;
...
antlrcpp::Any Pass2Visitor::visitVariableExpr(munchiesParser::VariableExprContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitSignedNumber(munchiesParser::SignedNumberContext *ctx)
{
    auto value = visitChildren(ctx);
    TypeSpec *type = ctx->number()->type;

    if (ctx->sign()->children[0] == ctx->sign()->SUB())
    {
...
antlrcpp::Any Pass2Visitor::visitIntegerConst(munchiesParser::IntegerConstContext *ctx)
{
    // Emit a load constant instruction.
    j_file << "\tldc\t" << ctx->getText() << endl;
    return visitChildren(ctx);
}
antlrcpp::Any Pass2Visitor::visitCharConst(munchiesParser::CharConstContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitDo_while(munchiesParser::Do_whileContext *ctx)
{
	cout << "--> in While()" << endl;
...
antlrcpp::Any Pass2Visitor::visitIf_stmt(munchiesParser::If_stmtContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitRelOpExpr(munchiesParser::RelOpExprContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitFn_defn(munchiesParser::Fn_defnContext *ctx)
{
	 cout << "--> in FuncDef(): " << ctx->funcID()->getText() << endl;
...
antlrcpp::Any Pass2Visitor::visitFunction_call(munchiesParser::Function_callContext *ctx)
{
...
antlrcpp::Any Pass2Visitor::visitRtn_stmt(munchiesParser::Rtn_stmtContext *ctx)
{
	cout<<"--> in return()"<<endl;
	visit(ctx->expr());
	return NULL;
